...
public class JvmProbe {
  ...
  public JvmInfo jvmInfo() {
    ...
    for (int i = 0; i < info.gc.collectors.length; i++) {
      ...
      info.gc.collectors[i].name =
        getByGcName(gcMxBean.getName(), gcMxBean.getName());
  ...
  /**
   * Resolves the GC type by its memory pool name ({@link MemoryPoolMXBean#getName()}.
   */
  static String getByMemoryPoolName(String poolName, String defaultName) {
    if (
      "Eden Space".equals(poolName) ||
    ...
    if (
      "Survivor Space".equals(poolName) ||
