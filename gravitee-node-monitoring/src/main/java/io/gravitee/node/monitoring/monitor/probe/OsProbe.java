...
public class OsProbe {
  ...
  public double[] getSystemLoadAverage() {
    if (Constants.LINUX || Constants.FREE_BSD) {
      final String procLoadAvg = Constants.LINUX
        ? "/proc/loadavg"
        : "/compat/linux/proc/loadavg";
    ...
    try {
      double oneMinuteLoadAverage = (double) getSystemLoadAverage.invoke(
        osMxBean
      );
      return new double[] {
        oneMinuteLoadAverage >= 0 ? oneMinuteLoadAverage : -1,
        -1,
        -1,
      };
    } catch (Exception ex) {
  ...
  private static double[] readProcLoadavg(String procLoadavg) {
    try {
      List<String> lines = Files.readAllLines(
        FileSystems.getDefault().getPath(procLoadavg)
      );
      if (!lines.isEmpty()) {
