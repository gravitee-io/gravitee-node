...
/**
 * Holds a {@link HazelcastInstance} through Spring's {@code @Lazy} proxy so construction of this
 * provider does not boot Hazelcast. The proxy dereferences on the first {@link #get(String)} call,
 * so when no consumer ever asks for a {@link DistributedMap} (e.g. {@code cluster.type=standalone}
 * + {@code ratelimit.type!=hazelcast}), Hazelcast stays dormant.
 */
