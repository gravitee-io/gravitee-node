...
public class RedisConfigurationProvider {
    ...
    public static RedisClientOptions from(Environment environment, String propertiesPrefix) {
        ...
        if (isSentinelEnabled(environment, propertiesPrefix)) {
            ...
            // The mapper only switches to SENTINEL mode when enabled is true; set it explicitly
            // rather than relying on the library builder default.
            sentinelBuilder.enabled(true);
    ...
    private static Optional<Integer> ofIntProperty(Environment environment, String key) {
    ...
    private static SslOptions loadSslOptions(Environment environment, String propertiesPrefix) {
        ...
        sslConfig.setOpenSsl(environment.getProperty(propertiesPrefix + ".openssl", Boolean.class, false));
        // hostnameVerifier: plugin-common-configurations stores a boolean; legacy YAML uses an
        // algorithm name (NONE / HTTPS / LDAPS / ...). Map NONE (or absent) to false, any other
        // value to true. Downstream the boolean coerces to "HTTPS" via
        // SslOptions.effectiveHostnameVerificationAlgorithm(), so non-{NONE,HTTPS} values are
        // silently treated as HTTPS — acceptable for Redis where HTTPS-style verification is
