...
public class KubernetesConfigMapKeyStoreLoader
  ...
  private static final Pattern CONFIGMAP_PATTERN = Pattern.compile(
    "^(.*/configmaps/.*)/.*$"
  );

  public KubernetesConfigMapKeyStoreLoader(
  ...
  public static boolean canHandle(KeyStoreLoaderOptions options) {
    ...
    return (
      ...
      kubernetesLocations
        .stream()
        .allMatch(location -> CONFIGMAP_PATTERN.matcher(location).matches())
    );
  }
  ...
  protected Completable loadKeyStore(ConfigMap configMap) {
    ...
    if (optResource.isEmpty()) {
      return Completable.error(
        new IllegalArgumentException("Unable to load keystore: unknown secret.")
      );
    }

    Map<String, String> data = configMap.getBinaryData() == null
      ? configMap.getData()
    ...
    if (data == null || data.get(dataKey) == null) {
      return Completable.error(
        new IllegalArgumentException(
          String.format(
            "No data has been found in the configmap for the specified key [%s].",
            dataKey
          )
        )
      );
