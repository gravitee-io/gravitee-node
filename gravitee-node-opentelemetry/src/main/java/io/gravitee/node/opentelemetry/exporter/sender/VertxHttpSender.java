...
import static io.gravitee.node.opentelemetry.exporter.OTelExporterUtil.getPort;
...
/**
 * See <a href="https://github.com/quarkusio/quarkus/blob/main/extensions/opentelemetry/runtime/src/main/java/io/quarkus/opentelemetry/runtime/exporter/otlp/sender/VertxHttpSender.java">VertxHttpSender.java</a>
 */
public final class VertxHttpSender implements HttpSender {
            ...
            .onFailure(
                new Predicate<Throwable>() {
                    ...
                    public boolean test(Throwable t) {
                        ...
                        return t instanceof IllegalStateException || t instanceof RejectedExecutionException || isShutdown.get();
                    }
                }
            )
            .recoverWithUni(
                new Supplier<Uni<? extends HttpClientRequest>>() {
                    @Override
    ...
    private static class ClientRequestSuccessHandler implements Handler<HttpClientRequest> {
        ...
        @Override
        public void handle(HttpClientRequest clientRequest) {
            clientRequest
                .response()
                        ...
                        clientResponse
                            ...
                            .onComplete(bodyResult -> {
                                if (bodyResult.succeeded()) {
                                    ...
                                    if (attemptNumber <= MAX_ATTEMPTS && !isShutdown.get()) {
                                        // retry
                                        initiateSend(client, requestURI, MAX_ATTEMPTS - attemptNumber, newAttempt(), onError, isShutdown);
                                    } else {
