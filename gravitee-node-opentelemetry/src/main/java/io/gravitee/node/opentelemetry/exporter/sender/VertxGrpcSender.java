...
import io.gravitee.node.opentelemetry.exporter.OTelExporterUtil;
...
public final class VertxGrpcSender implements GrpcSender {
    ...
    public void send(Marshaler request, Runnable onSuccess, BiConsumer onError) {
        ...
        initiateSend(client, server, MAX_ATTEMPTS, onSuccessHandler, throwable -> failOnClientRequest(marshalerType, throwable, onError));
    }
    ...
    private static void initiateSend(
        ...
        Uni
            ...
            .recoverWithUni(
                new Supplier<Uni<? extends GrpcClientRequest<Buffer, Buffer>>>() {
                    @Override
    ...
    private static final class ClientRequestOnSuccessHandler implements Handler<GrpcClientRequest<Buffer, Buffer>> {
        ...
        private void failOnClientRequest(Throwable t, BiConsumer<GrpcResponse, Throwable> onError, int attemptNumber) {
            final String message =
                "Failed to export " +
                type +
                "s. The request could not be executed after " +
                attemptNumber +
        ...
        public ClientRequestOnSuccessHandler newAttempt() {
            return new ClientRequestOnSuccessHandler(
                client,
                server,
                headers,
                compressionEnabled,
