...
public class OpenTelemetryTracerIntegrationTest {
    ...
    void should_connect_to_jaeger_over_grpc(Vertx vertx) throws Exception {
        ...
        var tracer = openTelemetryFactory.createTracer("serviceInstanceId", serviceName, "serviceNamespace", "serviceVersion");
        tracer.start();

        Context vertxContext = vertx.getOrCreateContext();
        vertxContext.runOnContext(v -> {
            var span = tracer.startSpanFrom(vertxContext, new InternalRequest("my-span", Map.of("custom", "value")));
            tracer.end(vertxContext, span);
        });
    ...
    private static OpenTelemetryFactory openTelemetryFactory(
        ...
        return new OpenTelemetryFactory(
            openTelemetryConfiguration,
            new ExporterFactory(openTelemetryConfiguration, vertx),
