...
|headers
|N/A
|Key-value pairs to be used as headers associated with exporter requests. Two list shapes are accepted: an inline entry whose key is the header name (`- X-Custom-Header: value`), or a `name`/`value` pair (`- name: X-Custom-Header` with `value: value`).
  ...
  opentelemetry:
    ...
    exporter:
      ...
      headers:
        ...
        # the name/value pair form is also accepted:
        - name: Authorization
