...
public class HeapDumpEndpoint implements ManagementEndpoint {
  ...
  public void handle(RoutingContext ctx) {
    ...
    final boolean live = Boolean.parseBoolean(
      ctx.request().getParam("live", Boolean.FALSE.toString())
    );
      ...
      .executeBlocking(
        new Handler<Promise<File>>() {
          ...
          public void handle(Promise<File> promise) {
            try {
              if (
                HeapDumpEndpoint.this.lock.tryLock(
                    HeapDumpEndpoint.this.timeout,
        ...
        new Handler<AsyncResult<File>>() {
          ...
          public void handle(AsyncResult<File> fileAsyncResult) {
            if (fileAsyncResult.succeeded()) {
                  ...
                  new Handler<AsyncResult<Void>>() {
                    ...
                    public void handle(AsyncResult<Void> voidAsyncResult) {
                      try {
                        ...
                        LOGGER.warn(
                          "Failed to delete temporary heap dump file '" +
                          file.toPath() +
                          "'",
                          ex
                        );
              ...
              LOGGER.error(
                "Unable to generate heap dump.",
                fileAsyncResult.cause()
              );
              response
