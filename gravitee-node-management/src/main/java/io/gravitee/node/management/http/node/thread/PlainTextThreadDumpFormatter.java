...
class PlainTextThreadDumpFormatter {
  ...
  private void writeStackTrace(
    ...
    for (StackTraceElement element : info.getStackTrace()) {
      writeStackTraceElement(
        ...
        element,
        info,
        lockedMonitorsForDepth(lockedMonitors, depth),
        depth == 0
      );
      depth++;
    }
  ...
  private List<MonitorInfo> lockedMonitorsForDepth(
    MonitorInfo[] lockedMonitors,
    int depth
  ) {
    return Stream
  ...
  private void writeStackTraceElement(
    ...
    if (firstElement && lockInfo != null) {
      if (
        element.getClassName().equals(Object.class.getName()) &&
  ...
  private void writeLockedOwnableSynchronizers(
    PrintWriter writer,
    ThreadInfo info
  ) {
