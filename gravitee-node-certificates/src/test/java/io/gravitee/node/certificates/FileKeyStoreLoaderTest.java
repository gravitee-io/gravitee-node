...
public class FileKeyStoreLoaderTest {
  ...
  public void shouldLoadPEMs() throws KeyStoreException {
    ...
    certificates.add(
      new CertificateOptions(getPath("localhost.cer"), getPath("localhost.key"))
    );
    certificates.add(
    ...
    certificates.add(
      new CertificateOptions(getPath("wildcard.cer"), getPath("wildcard.key"))
    );
  ...
  @Test
  public void shouldWatchFileAndDetectChanges()
    ...
    assertEquals(
      "localhost",
      KeyStoreUtils.getDefaultAlias(bundle.getKeyStore())
    );
