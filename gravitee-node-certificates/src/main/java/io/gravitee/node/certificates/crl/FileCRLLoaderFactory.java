...
public class FileCRLLoaderFactory implements CRLLoaderFactory {
    ...
    public boolean canHandle(CRLLoaderOptions options) {
        return (options != null && options.getPath() != null && !options.getPath().isEmpty() && new File(options.getPath()).isFile());
    }
    ...
    public CRLLoader create(CRLLoaderOptions options) {
        ...
        log.debug("Creating file CRL loader for options: {}", options);
        return new FileCRLLoader(options);
    }
}
