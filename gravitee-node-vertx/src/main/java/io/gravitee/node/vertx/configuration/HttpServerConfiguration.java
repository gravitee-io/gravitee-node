...
public class HttpServerConfiguration {
    ...
    public HttpServerOptions getHttpServerOptions() {
        ...
        if (this.isSecured()) {
            ...
            if (this.getTlsProtocols() != null) {
                options.setEnabledSecureTransportProtocols(new HashSet<>(Arrays.asList(this.getTlsProtocols().split("\\s*,\\s*"))));
            }

            // restrict the authorized ciphers
            if (this.getAuthorizedTlsCipherSuites() != null) {
                this.getAuthorizedTlsCipherSuites().stream().map(String::trim).forEach(options::addEnabledCipherSuite);
            }

            options.setClientAuth(this.getClientAuth());

            if (this.getTrustStorePaths() != null && !this.getTrustStorePaths().isEmpty()) {
                if (
                    ...
                    options.setTrustStoreOptions(
                        new JksOptions().setPath(this.getTrustStorePaths().get(0)).setPassword(this.getTrustStorePassword())
                    );
                } else if (this.getTrustStoreType().equalsIgnoreCase(CERTIFICATE_FORMAT_PEM)) {
                    final PemTrustOptions pemTrustOptions = new PemTrustOptions();
                    this.getTrustStorePaths().forEach(pemTrustOptions::addCertPath);
                    options.setPemTrustOptions(pemTrustOptions);
                } else if (this.getTrustStoreType().equalsIgnoreCase(CERTIFICATE_FORMAT_PKCS12)) {
                    options.setPfxTrustOptions(
                        new PfxOptions().setPath(this.getTrustStorePaths().get(0)).setPassword(this.getTrustStorePassword())
                    );
                }
            } else if (this.getTrustStoreType().equalsIgnoreCase(CERTIFICATE_FORMAT_SELF_SIGNED)) {
            ...
            if (this.getKeyStoreType().equalsIgnoreCase(CERTIFICATE_FORMAT_JKS)) {
                if (this.getKeyStorePath() == null || this.getKeyStorePath().isEmpty()) {
                    ...
                    options.setKeyStoreOptions(new JksOptions().setPath(this.getKeyStorePath()).setPassword(this.getKeyStorePassword()));
                }
            } else if (this.getKeyStoreType().equalsIgnoreCase(CERTIFICATE_FORMAT_PEM)) {
                if (this.getKeyStoreCertificates() == null || this.getKeyStoreCertificates().isEmpty()) {
            ...
            } else if (this.getKeyStoreType().equalsIgnoreCase(CERTIFICATE_FORMAT_PKCS12)) {
                if (this.getKeyStorePath() == null || this.getKeyStorePath().isEmpty()) {
                    logger.error("A PKCS#12 Keystore is missing. Skipping SSL keystore configuration...");
                } else {
                    options.setPfxKeyCertOptions(new PfxOptions().setPath(this.getKeyStorePath()).setPassword(this.getKeyStorePassword()));
                }
            } else if (this.getKeyStoreType().equalsIgnoreCase(CERTIFICATE_FORMAT_SELF_SIGNED)) {
                options.setPemKeyCertOptions(SelfSignedCertificate.create().keyCertOptions());
            }
        }

        if (this.isProxyProtocol()) {
            options.setUseProxyProtocol(true).setProxyProtocolTimeout(this.getProxyProtocolTimeout());
        }
        ...
        // Configure websocket
        System.setProperty("vertx.disableWebsockets", Boolean.toString(!this.isWebsocketEnabled()));
        if (this.isWebsocketEnabled() && this.getWebsocketSubProtocols() != null) {
            options.setWebSocketSubProtocols(new ArrayList<>(Arrays.asList(this.getWebsocketSubProtocols().split("\\s*,\\s*"))));
    ...
    public static class HttpServerConfigurationBuilder {
        ...
        public HttpServerConfigurationBuilder withDefaultKeyStoreCertificates(List<Certificate> keyStoreCertificates) {
        ...
        public HttpServerConfiguration build() {
            ...
            this.port = Integer.parseInt(environment.getProperty(prefix + "port", String.valueOf(port)));
            ...
            this.authorizedTlsCipherSuites = environment.getProperty(prefix + "ssl.tlsProtocols", List.class, authorizedTlsCipherSuites);

            String sClientAuthMode = environment.getProperty(prefix + "ssl.clientAuth", ClientAuth.NONE.name());
