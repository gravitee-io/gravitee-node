...
public abstract class AbstractVertxHttpServerFactory<T>
  ...
  public AbstractVertxHttpServerFactory(
    ...
    if (httpServerConfiguration.isSecured()) {
      if (keyStoreLoaderManager == null) {
        throw new IllegalArgumentException(
          "You must provide a KeyStoreLoaderManager when 'secured' is enabled."
        );
      }
      ...
      if (
        httpServerConfiguration.getTrustStorePaths() != null &&
        !httpServerConfiguration.getTrustStorePaths().isEmpty()
      ) {
        if (
          ...
          httpServerConfiguration
            .getTrustStorePaths()
            .forEach(pemTrustOptions::addCertPath);
      ...
      final KeyStoreLoaderOptions keyStoreLoaderOptions = KeyStoreLoaderOptions
        ...
        .withKeyStoreCertificates(
          httpServerConfiguration.getKeyStoreCertificates()
        )
        .withKubernetesLocations(
          httpServerConfiguration.getKeystoreKubernetes()
        )
      ...
      final VertxCertificateManager certificateManager = new VertxCertificateManager(
        httpServerConfiguration.isSni()
      );
      final KeyStoreLoader keyStoreLoader = keyStoreLoaderManager.create(
    ...
    if (httpServerConfiguration.isProxyProtocol()) {
      options
        ...
        .setProxyProtocolTimeout(
          httpServerConfiguration.getProxyProtocolTimeout()
        );
    }

    // Customizable configuration
    options.setHandle100ContinueAutomatically(
      httpServerConfiguration.isHandle100Continue()
    );
