 ...
 * @since 3.9.11
 */
public class GraviteePropertySource extends PropertySource<Map<String, Object>> {

    private static final Logger LOGGER = LoggerFactory.getLogger(GraviteePropertySource.class);
    private final PropertyResolverFactoriesLoader propertyResolverLoader;


    public GraviteePropertySource(String name, Map<String, Object> source, ApplicationContext applicationContext) {
        super(name, source);
        ...
        if (isCloudBased(value)) {
            for (PropertyResolver propertyResolver : propertyResolverLoader.getPropertyResolvers()) {
                if (propertyResolver.supports(value.toString())) {
                    ...
                    Object resolvedValue = propertyResolver.resolve(name, value.toString())
                            .doOnError(t -> LOGGER.error("Unable to resolve property {}", name, t))
                    ...
                    propertyResolver.watch(name, value.toString())
