...
class LoggingOverrideConfigurationTest {
    ...
    void should_configure_mdc_from_environment_properties() {
        MockEnvironment env = new MockEnvironment()
            .withProperty("node.logging.mdc.include[0]", "nodeId")
            .withProperty("node.logging.mdc.include[1]", "nodeHostname")
            .withProperty("node.logging.mdc.format", "[{key}={value}]")
            .withProperty("node.logging.mdc.nullValue", "N/A");
        ...
        assertThat(mdcConfig.getFormat()).isEqualTo("[{key}={value}]");
        assertThat(mdcConfig.getNullValue()).isEqualTo("N/A");
        assertThat(mdcConfig.getSeparator()).isEqualTo(" ");
    }

    @Test
    void should_not_override_patterns_when_flag_is_false() {
        MockEnvironment env = new MockEnvironment()
            .withProperty("node.logging.pattern.overrideLogbackXml", "false")
            .withProperty("node.logging.pattern.console", "%d %msg%n");
    ...
    void should_override_console_pattern_when_flag_is_true() {
        ...
        MockEnvironment env = new MockEnvironment()
            .withProperty("node.logging.pattern.overrideLogbackXml", "true")
            .withProperty("node.logging.pattern.console", newPattern);
    ...
    @Test
    void should_configure_custom_separator() {
        MockEnvironment env = new MockEnvironment()
            .withProperty("node.logging.mdc.include[0]", "nodeId")
            .withProperty("node.logging.mdc.separator", " | ");
