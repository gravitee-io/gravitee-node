...
class LoggingEndToEndTest {
    ...
    void should_render_mdcList_with_filtered_keys() {
        configureMdc(
            new MockEnvironment()
                ...
                .withProperty("node.logging.mdc.nullValue", "-")
        );
    ...
    void should_filter_individual_mdc_keys_via_percent_X() {
        configureMdc(new MockEnvironment().withProperty("node.logging.mdc.include[0]", "nodeId"));
    ...
    void should_use_null_value_for_missing_mdc_key_in_mdcList() {
        configureMdc(
            new MockEnvironment()
                .withProperty("node.logging.mdc.include[0]", "nodeId")
                .withProperty("node.logging.mdc.include[1]", "nonExistentKey")
    ...
    @Test
    void should_use_custom_separator_in_mdcList() {
        configureMdc(
            new MockEnvironment()
                .withProperty("node.logging.mdc.include[0]", "nodeId")
                .withProperty("node.logging.mdc.include[1]", "nodeHostname")
        ...
        String output = getOutput();
        assertThat(output).isEqualTo("nodeId: gw-1 | nodeHostname: host-alpha separated");
    }
