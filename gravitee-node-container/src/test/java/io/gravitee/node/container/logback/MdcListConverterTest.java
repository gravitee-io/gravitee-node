...
class MdcListConverterTest {
    ...
    void should_render_mdc_entries_in_declared_order() {
        NodeLoggerFactory.initMdcConfiguration(
            new MdcLoggingConfiguration(List.of("nodeId", "nodeHostname", "nodeApplication"), null, null, null)
        );
        ILoggingEvent event = mock(ILoggingEvent.class);
    ...
    void should_use_null_value_for_missing_mdc_entries() {
        NodeLoggerFactory.initMdcConfiguration(new MdcLoggingConfiguration(List.of("nodeId", "missing"), null, "-", null));
        ILoggingEvent event = mock(ILoggingEvent.class);
    ...
    void should_use_custom_format() {
        NodeLoggerFactory.initMdcConfiguration(new MdcLoggingConfiguration(List.of("nodeId"), "[{key}={value}]", null, null));
        ILoggingEvent event = mock(ILoggingEvent.class);
        when(event.getMDCPropertyMap()).thenReturn(Map.of("nodeId", "node-1"));
    ...
    void should_use_empty_string_as_default_null_value() {
        NodeLoggerFactory.initMdcConfiguration(new MdcLoggingConfiguration(List.of("missing"), null, null, null));
        ILoggingEvent event = mock(ILoggingEvent.class);
    ...
    void should_use_custom_separator() {
        NodeLoggerFactory.initMdcConfiguration(new MdcLoggingConfiguration(List.of("nodeId", "nodeHostname"), null, null, " | "));
        ILoggingEvent event = mock(ILoggingEvent.class);
        when(event.getMDCPropertyMap()).thenReturn(Map.of("nodeId", "node-1", "nodeHostname", "host-1"));

        String result = converter.convert(event);

        assertThat(result).isEqualTo("nodeId: node-1 | nodeHostname: host-1");
    }
}
